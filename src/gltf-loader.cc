...
bool LoadGLTF(const std::string &filename, float scale,
  ...
  for (const auto &gltfNode : model.nodes) {
      ...
      const auto& gltfMesh = model.meshes.at(gltfNode.mesh);
      ...
      {
          localTransX = gltfNode.translation[0];
          localTransY = gltfNode.translation[1];
          localTransZ = gltfNode.translation[2];
      }
    ...
    loadedMesh.localTranslate[0] = 0.0f;
    loadedMesh.localTranslate[1] = 0.0f;
    loadedMesh.localTranslate[2] = 0.0f;

    loadedMesh.localRotation[0] = 0.0f;
    loadedMesh.localRotation[1] = 0.0f;
    loadedMesh.localRotation[2] = 0.0f;

    loadedMesh.localScale[0] = 1.0f;
    loadedMesh.localScale[1] = 1.0f;
    loadedMesh.localScale[2] = 1.0f;

    if (gltfNode.translation.size() != 0)
    {
        loadedMesh.localTranslate[0] = gltfNode.translation[0];
        loadedMesh.localTranslate[1] = gltfNode.translation[1];
        loadedMesh.localTranslate[2] = gltfNode.translation[2];
    }
    
    if (gltfNode.rotation.size() != 0)
    {
        loadedMesh.localRotation[0] = gltfNode.rotation[0];
        loadedMesh.localRotation[1] = gltfNode.rotation[1];
        loadedMesh.localRotation[2] = gltfNode.rotation[2];
    }

    if (gltfNode.scale.size() != 0)
    {
        loadedMesh.localScale[0] = gltfNode.scale[0];
        loadedMesh.localScale[1] = gltfNode.scale[1];
        loadedMesh.localScale[2] = gltfNode.scale[2];
    }
    ...
    for (const auto &meshPrimitive : gltfMesh.primitives) {
        loadedMesh.material_ids = meshPrimitive.material;
      ...
      loadedMesh.scale[0] = fabs(pMax.x - pMin.x);
      loadedMesh.scale[1] = fabs(pMax.y - pMin.y);
      loadedMesh.scale[2] = fabs(pMax.z - pMin.z);

      loadedMesh.center[0] = bCenter.x + localTransX;
      loadedMesh.center[1] = bCenter.y + localTransY;
      loadedMesh.center[2] = bCenter.z + localTransZ;
  ...
  for (const auto& gltfMaterial : model.materials) 
  {
      ...
      loadedMaterial.base_texid = gltfMaterial.pbrMetallicRoughness.baseColorTexture.index;
      loadedMaterial.normal_texid = gltfMaterial.normalTexture.index;
      loadedMaterial.metallic_roughness_texid = gltfMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index;
  ...
  for (const auto &gltfTexture : model.textures) {
    ...
    loadedTexture.sampler.magFilter = model.samplers.at(gltfTexture.sampler).magFilter;
    loadedTexture.sampler.minFilter = model.samplers.at(gltfTexture.sampler).minFilter;
    loadedTexture.sampler.wrapS = model.samplers.at(gltfTexture.sampler).wrapS;
    loadedTexture.sampler.wrapT = model.samplers.at(gltfTexture.sampler).wrapT;
    loadedTexture.sampler.wrapR = model.samplers.at(gltfTexture.sampler).wrapR;
    const auto size =
